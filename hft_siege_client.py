...
MSG_AUTH_RESPONSE  = "auth_response"   # not a real server msg; auth ack is implicit
...
class HFTSiegeClient:
    ...
    async def connect(self) -> None:
        ...
        while True:
            try:
                logger.info("Connecting to %s (attempt %d)", self._cfg.url, attempt + 1)
    ...
    async def _dispatch(self, env: dict) -> None:
        ...
        if msg_type == MSG_PRICE_UPDATE and self._on_price_update:
            ...
            entries = [
                ...
                for i, e in enumerate(payload if isinstance(payload, list) else payload.get("entries", []))
            ]
            await self._invoke(self._on_leaderboard, entries)
